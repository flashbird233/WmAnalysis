...
def get_demand_amount(data, base,  eff, total_days, passed_days, keep_days):
    ...
    data['基础户达标来款金额'] = data.apply(lambda x: ((base * total_days - x['新年日均'] * passed_days) / keep_days) - x['新时点'], axis=1)
    # 有效户达标需来款金额 = (（eff * total_days - 新年日均 * passed_days） / keep_days) - 新时点
    data['有效户达标来款金额'] = data.apply(lambda x: ((eff * total_days - x['新年日均'] * passed_days) / keep_days) - x['新时点'], axis=1)
    return data
...
def get_down_base_table(data):
    data = data[['客户名', '管户经理', '账户销户', '基础户降级_标识', '旧年日均', '新年日均', '新时点',
                 '旧基础户达标天数', '新基础户达标天数', '基础户达标来款金额', '有效户达标来款金额']]
    # 筛选出客户基于标识的降级情况
...
def get_up_eff_table(data):
    data = data[['客户名', '管户经理', '有效户升级_标识', '旧年日均', '新年日均', '新时点',
                 '旧有效户达标天数', '新有效户达标天数', '有效户达标来款金额', '基础户达标来款金额']]
    # 筛选出客户基于标识的升级情况
...
def get_warning_base_table(data):
    # 仅保留有用列
    data = data[['客户名', '管户经理', '旧年日均', '新年日均', '新时点', '预警基础户', '基础户达标来款金额', '有效户达标来款金额']]
    # 仅保留预警基础户信息
...
def get_warning_eff_table(data):
    # 仅保留有用列
    data = data[['客户名', '管户经理', '旧年日均', '新年日均', '新时点', '预警有效户', '基础户达标来款金额', '有效户达标来款金额']]
    # 仅保留预警有效户信息
...
def get_critical_base_table(data):
    # 仅保留有用列
    data = data[['客户名', '管户经理', '旧年日均', '新年日均', '新时点', '基础户临界', '基础户达标来款金额', '有效户达标来款金额']]
    # 仅保留临界基础户信息
...
def get_critical_eff_table(data):
    # 仅保留有用列
    data = data[['客户名', '管户经理', '旧年日均', '新年日均', '新时点', '有效户临界', '基础户达标来款金额', '有效户达标来款金额']]
    # 仅保留临界有效户信息
